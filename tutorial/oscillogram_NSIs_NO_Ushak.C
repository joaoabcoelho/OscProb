...
myPMNS.SetM1(m1);
  ...
  TString outPath = TString::Format("histogram_eta_mumu_%f_m1_%f_mh_%i_ch_%i.root",eta_mumu,m1, mh, ch);
  ...
  if (ch==1){h2->SetTitle(TString::Format(";E (GeV);cos#theta_{z}; |P_{%s%s}(NSIs,m_{1}=1E-3eV,#eta_{#mu#mu}=0.4)-P_{%s%s}(std)|", flv1.c_str(), flv2.c_str(),flv1.c_str(), flv2.c_str()));}
  else {h2->SetTitle(TString::Format(";E (GeV);cos#theta_{z}; |P_{#bar{%s}#bar{%s}}(NSIs,m_{1}=1E-3eV,#eta_{#mu#mu}=0.4)-P_{#bar{%s}#bar{%s}}(std)|", flv1.c_str(), flv2.c_str(),flv1.c_str(), flv2.c_str()));}
...
void oscillogram_NSIs_NO_Ushak(int flvf, int flvi, int mh, int ch){// main function
