...
Definition at line 303 of file Prem\+Model.\+cxx.
...
Definition at line 497 of file Prem\+Model.\+cxx.
...
497                                              \{
498 
499   \textcolor{comment}{// Start with the first path}
500   \hyperlink{structOscProb_1_1NuPath}{NuPath} mergedPath = p1;
501 
502   \textcolor{comment}{// Add the second length}
503   mergedPath.\hyperlink{structOscProb_1_1NuPath_af22660894b6e25cf835500381b155557}{length} += p2.\hyperlink{structOscProb_1_1NuPath_af22660894b6e25cf835500381b155557}{length};
504 
505   \textcolor{comment}{// Compute weighted average of density}
506   mergedPath.\hyperlink{structOscProb_1_1NuPath_a54ddd451db69bc54434de3cf18a117ca}{density} = (p1.\hyperlink{structOscProb_1_1NuPath_a54ddd451db69bc54434de3cf18a117ca}{density}*p1.\hyperlink{structOscProb_1_1NuPath_af3213f3691ba83c6bc05f4a3490f6b31}{zoa}*p1.\hyperlink{structOscProb_1_1NuPath_af22660894b6e25cf835500381b155557}{length} + p2.
...
507 
508   \textcolor{comment}{// return merged path}
509   \textcolor{keywordflow}{return} mergedPath;
510 
511 \}
\end{DoxyCode}
...
Definition at line 325 of file Prem\+Model.\+cxx.
...
326 \{
327 
328   \textcolor{comment}{// Clear current path sequence}
329   \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}.clear();
330   
331   \textcolor{comment}{// Do nothing if cosine is unphysical}
332   \textcolor{keywordflow}{if}(fabs(cosT) > 1) \textcolor{keywordflow}{return} 0;
333 
334   \textcolor{comment}{// Define the minimum path radius}
335   \textcolor{keywordtype}{double} minR = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[\hyperlink{classOscProb_1_1PremModel_a4fb68506493666349f418b893a996185}{fDetLayer}].radius * sqrt(1 - cosT*cosT);
336 
337   \textcolor{comment}{// Set the top layer index}
338   \textcolor{keywordtype}{int} toplayer = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}.size() - 1;
339   
340   \textcolor{comment}{// Find the inner-most crossed layer}
341   \textcolor{keywordtype}{int} minlayer = 0;
342   \textcolor{keywordflow}{while}(\hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[minlayer].radius < minR) minlayer++; 
343 
344   \textcolor{comment}{// Compute the number of path segments needed}
345   \textcolor{keywordtype}{int} nsteps = toplayer - \hyperlink{classOscProb_1_1PremModel_a4fb68506493666349f418b893a996185}{fDetLayer};
346   \textcolor{keywordflow}{if}(cosT < 0) nsteps += 2*(fDetLayer - minlayer) + 1;
347 
348   \textcolor{comment}{// Start at the top layer and go down}
349   \textcolor{keywordtype}{int} layer = toplayer;
350   \textcolor{keywordtype}{int} dl = -1;
351   
352   \textcolor{comment}{// Loop over all path segments}
353   \textcolor{keywordflow}{for}(\textcolor{keywordtype}{int} i=0; i<nsteps; i++)\{
354   
355     \textcolor{comment}{// Get square of the path length between this layer's }
356     \textcolor{comment}{// outer radius and  inner-most radius}
357     \textcolor{keywordtype}{double} L1 = pow(\hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[layer].radius,2) - minR*minR;
358 
359     \textcolor{comment}{// If L1 is negative, outer radius is not crossed.}
360     \textcolor{comment}{// This only happens if detector is at the top layer and the}
361     \textcolor{comment}{// neutrino is coming from above.}
362     \textcolor{keywordflow}{if}(L1 < 0) \textcolor{keywordflow}{return} \textcolor{keyword}{true};
363     
364     \textcolor{comment}{// Get square of the path length between this layer's }
365     \textcolor{comment}{// inner radius and  inner-most radius}
366     \textcolor{keywordtype}{double} L2 = -minR*minR;
367     \textcolor{keywordflow}{if}(layer>0) L2 += pow(\hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[layer-1].radius,2);
368     
369     \textcolor{comment}{// If L2 is negative, inner radius is not crossed,}
370     \textcolor{comment}{// so set this as the minimum layer.}
371     \textcolor{keywordtype}{bool} ismin = (L2<=0 && cosT < 0);
372 
373     \textcolor{comment}{// Store the path segment length}
374     \textcolor{keywordtype}{double} dL;
375     
376     \textcolor{comment}{// If it's the minimum layer, connect two outer radius }
377     \textcolor{comment}{// crossing points. If not, compute difference between}
378     \textcolor{comment}{// inner and outer radius crossings.}
379     \textcolor{keywordflow}{if}(ismin)      dL = 2 * sqrt(L1);
380     \textcolor{keywordflow}{else} \textcolor{keywordflow}{if}(L2>=0) dL = sqrt(L1) - sqrt(L2);
381     \textcolor{keywordflow}{else}           dL = sqrt(L1); \textcolor{comment}{// This should never actually happen,}
382                                   \textcolor{comment}{// but protect in case L2 is slightly}
383                                   \textcolor{comment}{// negative when it should be zero.}
384                                   \textcolor{comment}{// e.g. arriving at the detector from above.}
385     
386     \textcolor{comment}{// Add this path segment to the sequence}
387     \hyperlink{classOscProb_1_1PremModel_aca013f7ac5494282834048786a0e07a6}{AddPath}(dL, \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[layer]);
388     
389     \textcolor{comment}{// If we reached the inner-most layer,}
390     \textcolor{comment}{// start moving up again.}
391     \textcolor{keywordflow}{if}(ismin) dl = 1;
392     
393     \textcolor{comment}{// Move to next layer}
394     layer += dl;
395   
396   \}
397 
398   \textcolor{comment}{// Return the number of path segments}
399   \textcolor{keywordflow}{return} \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}.size();
400 
401 \}
\end{DoxyCode}
...
Definition at line 281 of file Prem\+Model.\+cxx.
...
282 \{
283 
284   \textcolor{keywordtype}{double} rAbove = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}.back().radius;     \textcolor{comment}{// Radius above detector}
285   \textcolor{keywordtype}{double} rBelow = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[\hyperlink{classOscProb_1_1PremModel_a4fb68506493666349f418b893a996185}{fDetLayer}].radius; \textcolor{comment}{// Radius below detector}
286   
287   \textcolor{keywordflow}{if}(L < rAbove - rBelow) \textcolor{keywordflow}{return}  1;
288   \textcolor{keywordflow}{if}(L > rAbove + rBelow) \textcolor{keywordflow}{return} -1;
289   
290   \textcolor{keywordflow}{return} (rAbove*rAbove - rBelow*rBelow - L*L) / (2*rBelow*L);
291   
292 \}
\end{DoxyCode}
...
Definition at line 416 of file Prem\+Model.\+cxx.
...
416                                                    \{
417 
418   \textcolor{comment}{// The output vector}
419   vector<NuPath> mergedPath;
420 
421   \textcolor{comment}{// Start with the first path}
422   \hyperlink{structOscProb_1_1NuPath}{OscProb::NuPath} path = \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[0];
423 
424   \textcolor{comment}{// Track the total length}
425   \textcolor{keywordtype}{double} totL = 0;
426 
427   \textcolor{comment}{// Loop over all paths starting from second}
428   \textcolor{keywordflow}{for}(\textcolor{keywordtype}{int} i=1; i<\hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}.size(); i++)\{
429 
430     \textcolor{comment}{// If this path electron density is beyond the tolerance}
431     \textcolor{keywordflow}{if}( fabs(path.\hyperlink{structOscProb_1_1NuPath_a54ddd451db69bc54434de3cf18a117ca}{density}*path.\hyperlink{structOscProb_1_1NuPath_af3213f3691ba83c6bc05f4a3490f6b31}{zoa} - \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[i].density*\hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[i].zoa) > prec*path
      .\hyperlink{structOscProb_1_1NuPath_af3213f3691ba83c6bc05f4a3490f6b31}{zoa} )\{
432 
433       \textcolor{comment}{// Add merged path to vector}
434       mergedPath.push\_back(path);
435       
436       \textcolor{comment}{// Set this path as new merged path}
437       path = \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[i];
438 
439     \}
440     \textcolor{comment}{// If path is within tolerance}
441     \textcolor{keywordflow}{else}\{
442       
443       \textcolor{comment}{// Merge the path with current merged path}
444       path = \hyperlink{classOscProb_1_1PremModel_a646977424cdca178a77694397146c2f8}{AvgPath}(path, \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[i]);
445 
446     \}
447 
448     \textcolor{comment}{// Increment total length}
449     totL += \hyperlink{classOscProb_1_1PremModel_aaf3c77e35798d664853157013c90ad2b}{fNuPath}[i].length;
450 
451   \}\textcolor{comment}{// End of loop over paths}
452 
453   \textcolor{comment}{// Add the final merged path to vector}
454   mergedPath.push\_back(path);
455 
456   \textcolor{comment}{// If tag is true, remove small paths}
457   \textcolor{keywordflow}{if}(\hyperlink{classOscProb_1_1PremModel_a3973df6f5f2ff219cd2f865b31aacfd2}{fRemoveSmallPaths})\{
458 
459     \textcolor{comment}{// Start at first path}
460     \textcolor{keywordtype}{int} k = 0;
461 
462     \textcolor{comment}{// While not at the end of vector}
463     \textcolor{keywordflow}{while}(k + 1 < mergedPath.size())\{
464 
465       \textcolor{comment}{// If length is less than 1% of total}
466       \textcolor{keywordflow}{if}(mergedPath[k].length < 0.01*totL)\{
467 
468         \textcolor{comment}{// Merge path with following path}
469         mergedPath = \hyperlink{classOscProb_1_1PremModel_a87cb8043b58fde2c8a8779e6ae1b5135}{MergePaths}(mergedPath, k, k+1);
470 
471       \}
472       \textcolor{comment}{// If path is long enough skip it}
473       \textcolor{keywordflow}{else} k++;
474 
475     \}\textcolor{comment}{// End of while loop}
476 
477   \}\textcolor{comment}{// End of if statement}
478 
479   \textcolor{comment}{// return the merged vector}
480   \textcolor{keywordflow}{return} mergedPath;
481 
482 \}
\end{DoxyCode}
...
256 \{
257 
258   \textcolor{keywordflow}{if}(fabs(cosT) > 1) \textcolor{keywordflow}{return} 0;
259 
260   \textcolor{keywordtype}{double} rAbove = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}.back().radius;     \textcolor{comment}{// Radius above detector}
261   \textcolor{keywordtype}{double} rBelow = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[\hyperlink{classOscProb_1_1PremModel_a4fb68506493666349f418b893a996185}{fDetLayer}].radius; \textcolor{comment}{// Radius below detector}
262 
263   \textcolor{keywordtype}{double} sinsqrT = 1 - cosT*cosT;
264 
265   \textcolor{keywordflow}{return} -rBelow*cosT + sqrt(rAbove*rAbove - rBelow*rBelow*sinsqrT);
266 
267 \}
\end{DoxyCode}
...
Definition at line 521 of file Prem\+Model.\+cxx.
...
521                                                                           \{
522 
523   \textcolor{comment}{// Output vector}
524   vector<NuPath> mergedPath;
525 
526   \textcolor{comment}{// Loop over input paths}
527   \textcolor{keywordflow}{for}(\textcolor{keywordtype}{int} i=0; i<inputPath.size(); i++)\{
528     
529     \textcolor{comment}{// If first index, merge the paths j and k}
530     \textcolor{keywordflow}{if}(i==j) mergedPath.push\_back(\hyperlink{classOscProb_1_1PremModel_a646977424cdca178a77694397146c2f8}{AvgPath}(inputPath[j], inputPath[k]));
531     \textcolor{comment}{// If not second index add the path as is}
532     \textcolor{keywordflow}{else} \textcolor{keywordflow}{if}(i!=k) mergedPath.push\_back(inputPath[i]);
533 
534   \}\textcolor{comment}{// End of loop}
535 
536   \textcolor{comment}{// return merged vector}
537   \textcolor{keywordflow}{return} mergedPath;
538 
539 \}
\end{DoxyCode}
...
Definition at line 547 of file Prem\+Model.\+cxx.
...
\begin{DoxyCode}
547 \{ \hyperlink{classOscProb_1_1PremModel_a3973df6f5f2ff219cd2f865b31aacfd2}{fRemoveSmallPaths} = rp; \}
\end{DoxyCode}
...
Definition at line 222 of file Prem\+Model.\+cxx.
...
223 \{
224 
225   \textcolor{keywordflow}{if}(thick <= 0)\{
226     cout << \textcolor{stringliteral}{"Layer thickness should be positive. Do nothing."} << endl;
227     \textcolor{keywordflow}{return};
228   \}
229 
230   \textcolor{keywordtype}{int} nlayers = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}.size();
231   
232   \textcolor{keywordflow}{if}(nlayers < 1)\{
233     cout << \textcolor{stringliteral}{"PremModel has no layers. Do nothing."} << endl;
234     \textcolor{keywordflow}{return};
235   \}
236   
237   \textcolor{keywordtype}{double} bottomRadius;
238   \textcolor{keywordflow}{if}(nlayers>1) bottomRadius = \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[nlayers-2].radius;
239   \textcolor{keywordflow}{else}          bottomRadius = 0;
240 
241   \textcolor{keywordtype}{double} topRadius = bottomRadius + thick;
242   
243   \hyperlink{classOscProb_1_1PremModel_a19a9a3b23ec154ad7a29f92b74aa5bc6}{fPremLayers}[nlayers-1].radius = topRadius;
244   
245 \}
\end{DoxyCode}
...
Definition at line 86 of file Prem\+Model.\+h.
...
Referenced by Add\+Layer(), Clear\+Model(), Fill\+Path(), Get\+Cos\+T(), Get\+Prem\+Layers(), Get\+Total\+L(), Load\+Model(), Set\+Layer\+Zo\+A(), and Set\+Top\+Layer\+Size().
