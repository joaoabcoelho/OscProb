...
namespace OscProb {
  ...
  class PMNS_SiderealLIV : public PMNS_Fast {
    public:
      ...
      /// All three arguments carry the sign: negative for South, positive for
      /// North.
    ...
    protected:
      ...
      /// @param coordName - human-readable name for the warning ("coord",
      /// "coord1", ...)
      ...
      // ------------------------------------------------------------------------
      // SME coefficients (real; only the upper triangle in flavour indices is
      // used)
      // ------------------------------------------------------------------------
    ...
    private:
      double fChi; ///< Colatitude of the detector (chi = 90 - latitude), in
                   ///< degrees
  };
