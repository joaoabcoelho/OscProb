...
namespace OscProb {
  ...
  struct NuPath {
      ...
      /// @param ly - An index to identify the matter type (e.g. earth inner
      /// core)
      ///
  ...
  struct PremLayer {
      ...
      {
        return rhs.density == density && rhs.zoa == zoa && rhs.layer == layer;
      }
  };

  NuPath AvgPath(NuPath& p1, NuPath& p2); ///< Get the average of two paths
  NuPath AvgPath(
      std::vector<NuPath>& pv); ///< Get the average of a vector of paths
  std::vector<NuPath> MergePaths(std::vector<NuPath>& inputPath, int j,
                                 int k); ///< Merge paths j and k in vector
